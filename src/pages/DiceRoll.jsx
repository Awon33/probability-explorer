...
const DiceRoll = () => {
  ...
  const getNextQuestion = (targetDifficulty, excludeIds) => {
    const available = diceQuestions.filter(q => q.difficulty === targetDifficulty && !excludeIds.includes(q.id));

    if (available.length === 0) {
      const anyAvailable = diceQuestions.filter(q => !excludeIds.includes(q.id));
  ...
  const finishGame = () => {
    navigate('/summary', { state: { game: 'dice', score, totalQuestions: questionsAskedCount } });
  };
      ...
      <div className="relative z-0 max-w-6xl mx-auto px-3 sm:px-4 lg:px-8">
            ...
            <div className="text-center flex-1 lg:flex-none">
              <div className="flex items-center justify-center gap-2 lg:gap-3 mb-1 lg:mb-2">
                <div className={`w-8 h-8 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl bg-gradient-to-r ${getHeaderColor()} flex items-center justify-center shadow-lg`}>
                  <Target size={16} lg:size={24} className="text-white" />
                </div>
                <h1 className="text-2xl lg:text-4xl font-bold bg-gradient-to-r from-gray-900 to-purple-700 bg-clip-text text-transparent">
                  Dice Roll
                </h1>
              </div>
              <p className="text-xs lg:text-base text-gray-600 hidden lg:block">Roll, learn, and master 6-sided probability</p>
              <p className="text-xs text-gray-600 lg:hidden">6-Sided Probability Lab</p>
            </div>
          ...
          <div className="lg:hidden mb-6">
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-sm p-3 text-center border border-gray-100">
                ...
                <p className="text-xs text-gray-500 mt-1">Rolls</p>
              </div>
        ...
        <main className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 pb-8 lg:pb-12">
          ...
          <div className="lg:col-span-2 space-y-6 lg:space-y-8">
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-2xl lg:rounded-3xl shadow-xl lg:shadow-2xl p-6 lg:p-8 border border-gray-100">
              <div className="flex flex-col items-center">
                ...
                <button
                  onClick={rollDice}
                  disabled={isRolling || showModal}
                  className={`group relative w-full lg:w-auto px-8 lg:px-14 py-4 lg:py-5 text-lg lg:text-2xl font-bold text-white rounded-xl lg:rounded-2xl shadow-xl lg:shadow-2xl transition-all duration-300 transform hover:scale-105 active:scale-95 overflow-hidden ${isRolling || showModal
                      ? 'bg-gradient-to-r from-gray-400 to-gray-500 cursor-not-allowed'
                      : 'bg-gradient-to-r from-purple-600 via-blue-600 to-purple-700 hover:shadow-3xl'
                    }`}
                >
                  <div className="relative z-10 flex items-center justify-center gap-2 lg:gap-3">
                    {isRolling ? (
                      ...
                      <>
                        ...
                        <span className="lg:hidden">Roll Dice</span>
                        <span className="hidden lg:inline">Roll Dice Now</span>
                ...
                <div className="mt-4 lg:mt-6 text-center">
                  <p className="text-xs lg:text-sm text-gray-500 mb-2">
                    <span className="font-semibold text-purple-600">Tip:</span> Every 3rd roll triggers a question
                  </p>
                  <div className="inline-flex items-center gap-1 lg:gap-2 text-xs lg:text-sm bg-gradient-to-r from-purple-50 to-blue-50 px-3 lg:px-4 py-1 lg:py-2 rounded-full">
                    <Target size={12} lg:size={14} className="text-purple-600" />
            ...
            <div className="bg-white/90 backdrop-blur-sm rounded-2xl lg:rounded-3xl shadow-xl lg:shadow-2xl p-6 lg:p-8 border border-gray-100">
              <h2 className="text-xl lg:text-2xl font-bold text-gray-900 mb-4 lg:mb-6 flex items-center gap-2 lg:gap-3">
                <BarChart className="text-purple-600" size={20} lg:size={24} />
                Dice Distribution
              </h2>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 lg:gap-4">
                {[1, 2, 3, 4, 5, 6].map((num) => (
                  <div
                    key={num}
                    className={`p-4 lg:p-6 rounded-xl lg:rounded-2xl text-center transition-all duration-300 ${number === num
                        ? 'bg-gradient-to-br from-purple-50 to-blue-50 border-2 border-purple-300 shadow-lg'
                        : 'bg-gray-50 border border-gray-100'
                      }`}
                  >
                    <div className={`text-2xl lg:text-3xl font-bold ${number === num ? 'text-purple-700' : 'text-gray-700'} mb-1 lg:mb-2`}>
                      {num}
                    </div>
                    <div className="text-sm lg:text-base font-bold mb-1">{stats[num]}</div>
                    <div className="text-xs text-gray-500">times</div>
                    <div className="mt-2 lg:mt-3 text-xs lg:text-sm text-gray-600">
                      {getPercentage(stats[num])}%
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-6 lg:space-y-8">
            ...
            <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl lg:rounded-3xl shadow-xl lg:shadow-2xl p-6 lg:p-8 border border-gray-100">
              ...
              <div className="space-y-4 lg:space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3 lg:gap-4">
                  <div className="bg-gradient-to-br from-purple-50 to-blue-50 p-3 lg:p-4 rounded-xl lg:rounded-2xl">
                    <TrendingUp className="text-purple-600 mb-1 lg:mb-2" size={16} lg:size={20} />
                ...
                <div className="p-3 lg:p-4 bg-gradient-to-r from-purple-50/50 to-blue-50/50 rounded-xl lg:rounded-2xl border border-purple-100">
                  <p className="text-xs lg:text-sm text-gray-700">
                    <span className="font-semibold text-purple-600">How it works:</span> Answer questions to level up!
                  </p>
            ...
            <div className="bg-gradient-to-br from-white to-gray-50 rounded-2xl lg:rounded-3xl shadow-xl lg:shadow-2xl p-6 lg:p-8 border border-gray-100">
              <h2 className="text-lg lg:text-xl font-bold text-gray-900 mb-3 lg:mb-4">Dice Facts</h2>
              <div className="space-y-3 lg:space-y-4">
                <div className="p-3 lg:p-4 bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl lg:rounded-2xl">
                  ...
                  <p className="text-xs text-gray-600 mt-1">Each number has 1/6 (16.67%) chance</p>
                </div>
                <div className="p-3 lg:p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl lg:rounded-2xl">
                  <p className="text-sm font-medium text-gray-800">Expected Value</p>
                  <p className="text-xs text-gray-600 mt-1">Average roll = 3.5</p>
                </div>
                <div className="p-3 lg:p-4 bg-gradient-to-r from-purple-50 to-violet-50 rounded-xl lg:rounded-2xl">
                  <p className="text-sm font-medium text-gray-800">Independence</p>
                  <p className="text-xs text-gray-600 mt-1">Each roll is independent of others</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl p-4 border border-gray-200 shadow-md">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 lg:gap-3">
                <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 flex items-center justify-center">
                  <span className="text-white font-bold text-sm lg:text-base">P</span>
                </div>
                <div>
                  <p className="text-sm lg:font-medium text-gray-900">ProbX Dice Lab</p>
                  <p className="text-xs text-gray-500">6-sided probability learning</p>
                </div>
              </div>
                ...
                <div className="text-right">
                  <div className="flex items-center gap-2 lg:gap-3">
                    <div className="text-center">
                      ...
                      <p className="text-[10px] text-gray-500">Rolls</p>
                    </div>
